...
const Appointment = {
    create: (data, callback) => {
        const sql = `INSERT INTO appointments (name, email, number, age, assessmentType, selectData, slot, paymentDetails ) 
                     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`;
        db.query(sql, [
            data.name,
            data.email,
    ...
    update: (id, data, callback) => {
        const sql = `UPDATE appointments SET name = ?, email = ?, number = ?, age = ?, assessmentType = ?, selectData = ?, 
